...
- **Disable Proxy 按钮** — Actions 卡片新增按钮，一键删除 opencode.json 的 `shell` 字段（仅当指向 tg-shell.cmd），恢复为默认 PowerShell；"Setup OpenCode" 按钮一键恢复代理。按钮文案随状态变化（启用=Disable Proxy / 禁用=Proxy Disabled 半透明）
