...
=== ${f} passed, ${v} failed ===`),v===0?u.window.showInformationMessage(u.l10n.t("OpenCode Grid API: All {0} tests passed!",f)):u.window.showWarningMessage(u.l10n.t("OpenCode Grid API: {0} test(s) failed. See output.",v))}),)}function Be(){try{let dir=U.join(me.homedir(),".opencode-grid","ports");if(P.existsSync(dir)){let files=P.readdirSync(dir).filter(f=>f.startsWith("win-")&&f.endsWith(".json"));for(let f of files)try{let fp=U.join(dir,f);let d=JSON.parse(P.readFileSync(fp,"utf-8"));if(d.extHostPid===process.pid)P.unlinkSync(fp)}catch(e){}}}catch(e){}H?.stop(),H=void 0,w.disposeAll()}0&&(module.exports={activate,deactivate});
