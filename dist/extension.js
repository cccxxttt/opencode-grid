...
<head>
  ...
  <style>
    .oc-cell-denied{opacity:0.5;pointer-events:none;border-color:#e33!important}
    .oc-cell-hint{font-size:11px;color:#e33;margin-top:2px;transition:opacity .3s}
    ${t}
...
<body>
  ...
  <script nonce="${s}">
    ...
    var __GRID_ZOOM = ${y.workspace.getConfiguration("opencodeGrid").get("zoomPercent",100)};
    var __GRID_FONT_FAMILY = ${JSON.stringify(y.workspace.getConfiguration("opencodeGrid").get("fontFamily",""))};
    var __GRID_BG_COLOR = ${JSON.stringify(y.workspace.getConfiguration("opencodeGrid").get("backgroundColor",""))};
    var __GRID_FG_COLOR = ${JSON.stringify(y.workspace.getConfiguration("opencodeGrid").get("foregroundColor",""))};
    var __GRID_THEME = ${JSON.stringify(y.workspace.getConfiguration("opencodeGrid").get("colorTheme",""))};
    var __GRID_THEME_COLORS = ${JSON.stringify(A(y.workspace.getConfiguration("opencodeGrid").get("colorTheme","")))};
...
<body>
  <div class="container">
    ...
    <!-- Custom API -->
    <div class="glass-card" data-section="customApi">
      <div class="section-header collapsible">
        <div class="section-label">${i.l10n.t("Custom API")}</div>
        <span class="tip-wrap">
          <span class="tip-icon">?</span>
          <div class="tip-bubble">
            ${i.l10n.t("Manage custom OpenAI-compatible API providers. Add a provider with its base URL, API key, and models — opencode-grid writes them to ~/.config/opencode/opencode.json. Restart opencode after changes.")}
          </div>
        </span>
        <span class="collapse-icon">▾</span>
      </div>
      <div class="section-body">
        <div style="font-size:11px;opacity:.7;margin-bottom:8px">${i.l10n.t("Add or manage custom API providers. Changes apply to opencode.json — restart opencode to take effect.")}</div>
        <div id="customApiProviderList" style="display:flex;flex-direction:column;gap:4px;margin-bottom:8px"></div>
        <button class="glass-btn" id="apiAddProvBtn" style="margin-bottom:6px">+ ${i.l10n.t("Add Provider")}</button>
        <div id="apiAddForm" style="display:none;border-top:1px solid rgba(255,255,255,.06);padding-top:8px">
          <div style="display:flex;flex-direction:column;gap:4px">
            <input class="glass-input" id="apiName" placeholder="${i.l10n.t("Provider name (e.g. My Custom GPT)")}" style="width:100%" />
            <input class="glass-input" id="apiId" placeholder="${i.l10n.t("Provider ID (auto from name, editable)")}" style="width:100%" />
            <input class="glass-input" id="apiBase" placeholder="${i.l10n.t("Base URL (e.g. https://api.openai.com/v1)")}" style="width:100%" />
            <div style="display:flex;gap:4px;align-items:center">
              <input class="glass-input" id="apiKey" type="password" placeholder="${i.l10n.t("API Key (plaintext, or {env:VAR} / {file:path})")}" style="flex:1" />
            </div>
            <div style="font-size:11px;opacity:.7;margin-top:4px;">${i.l10n.t("Models:")}</div>
            <div id="apiModels" style="display:flex;flex-direction:column;gap:4px"></div>
            <button class="glass-btn" id="apiAddModelBtn" style="margin-top:4px">+ ${i.l10n.t("Add model")}</button>
            <button class="glass-btn" id="apiSaveBtn" style="margin-top:8px">${i.l10n.t("Save Provider")}</button>
            <button class="glass-btn" id="apiCancelBtn" style="margin-top:4px">${i.l10n.t("Cancel")}</button>
          </div>
        </div>
        <div style="margin-top:8px;display:flex;align-items:center;gap:4px">
          <span style="font-size:11px;opacity:.7">${i.l10n.t("Active model:")}</span>
          <select id="apiActiveModel" class="glass-select" style="flex:1;min-width:0">
            <option value="">${i.l10n.t("(not set)")}</option>
          </select>
        </div>
      </div>
    </div>
    ...
    <div class="pty-banner" id="ptyBanner">
      ...
      <span class="pty-banner-text">${i.l10n.t("node-pty is required to use OpenCode Grid.")}</span>
    ...
    <div class="glass-card" data-section="actions">
      ...
      <div class="section-body">
        <div class="btn-group">
          ...
          <button class="glass-btn" id="setupOpencodeBtn">
            <span class="btn-icon">&#9881;</span> ${i.l10n.t("Setup OpenCode")}
          </button>
          <button class="glass-btn" id="disableProxyBtn">
            <span class="btn-icon">&#9211;</span> ${i.l10n.t("Disable Proxy")}
          </button>
        </div>
      </div>
    </div>
...
`,"<br>")}
  ...
  <script nonce="${e}">
    ...
    document.getElementById('setupOpencodeBtn').addEventListener('click', function() {
      vscode.postMessage({ type: 'setupOpencode' });
    });

    var disableProxyBtn = document.getElementById('disableProxyBtn');
    if (disableProxyBtn) {
      disableProxyBtn.addEventListener('click', function() {
        vscode.postMessage({ type: 'disableShellProxy' });
      });
    }
   ...
   // \u2500\u2500 Cell Merge preview grid \u2500\u2500
    ...
    // \u2500\u2500 Messages \u2500\u2500
    document.getElementById('ocCellSelect').addEventListener('change',function(){vscode.postMessage({type:'selectCell',cellId:parseInt(this.value)})});
    function showSelectCellDenied(reason){var sel=document.getElementById('ocCellSelect');if(!sel)return;sel.classList.add('oc-cell-denied');var hint=document.getElementById('ocCellHint');if(!hint){hint=document.createElement('div');hint.id='ocCellHint';hint.className='oc-cell-hint';sel.parentNode.appendChild(hint)}hint.textContent=reason||'Cell switch denied';setTimeout(function(){sel.classList.remove('oc-cell-denied');if(hint)hint.textContent=''},3000)}

    function updateCellSelector(msg){var sel=document.getElementById('ocCellSelect'),count=msg.cellCount||0,labels=msg.cellLabels||{},val=typeof msg.selectedCell==='number'?msg.selectedCell:parseInt(sel.value)||-1;sel.innerHTML='<option value="-1">opencode</option>';for(var i=0;i<count;i++){var opt=document.createElement('option');opt.value=msg.cellIds?.[i]??i;opt.textContent=labels[i]||('Cell '+i);sel.appendChild(opt)}sel.value=sel.querySelector('option[value="'+val+'"]')?val:'-1'}
...
if (msg.type === 'selectCellDenied') {
        showSelectCellDenied(msg.reason);
      }
      ...
      if (msg.type === 'configValues') {
        ...
        var curHiddenCells = msg.hiddenCells || [];if(typeof updateCellSelector==='function')updateCellSelector(msg);
...
if (msg.type === 'apiProviders') {
        renderApiProviders(msg.providers || []);
      }
      if (msg.type === 'apiModels') {
        renderApiModels(msg.models || []);
      }
      if (msg.type === 'shellProxyStatus') {
        var pbtn = document.getElementById('disableProxyBtn');
        if (pbtn) {
          if (msg.enabled) {
            pbtn.innerHTML = '<span class="btn-icon">&#9211;</span> Disable Proxy';
            pbtn.style.opacity = '1';
          } else {
            pbtn.innerHTML = 'Proxy Disabled';
            pbtn.style.opacity = '.5';
          }
        }
      }
    });

    // ── Custom API card: provider list + add form ──
    var apiProviders = [];
    var apiModels = [];
    function slugify(s) { return (s || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, ''); }
    function renderApiProviders(providers) {
      apiProviders = providers || [];
      var list = document.getElementById('customApiProviderList');
      if (!list) return;
      list.innerHTML = '';
      if (apiProviders.length === 0) {
        var empty = document.createElement('div');
        empty.className = 'cmd-empty';
        empty.textContent = 'No custom providers configured.';
        list.appendChild(empty);
        return;
      }
      apiProviders.forEach(function (p) {
        ...
        var txt = document.createElement('span');
        txt.className = 'cmd-item-text';
        txt.style.fontFamily = 'inherit';
        var modelCount = (p.models || []).length;
        txt.textContent = p.displayName || p.providerId;
        txt.title = p.baseURL || '';
        var meta = document.createElement('span');
        meta.style.cssText = 'font-size:10px;opacity:.55;';
        ...
        del.addEventListener('click', function (e) {
          e.stopPropagation();
          if (!confirm('Remove provider "' + (p.displayName || p.providerId) + '"?')) return;
          vscode.postMessage({ type: 'removeApiProvider', providerId: p.providerId });
        });
    ...
    function renderApiModels(models) {
      apiModels = models || [];
      var sel = document.getElementById('apiActiveModel');
      if (!sel) return;
      sel.innerHTML = '<option value="">(not set)</option>';
      apiModels.forEach(function (m) {
        var opt = document.createElement('option');
        opt.value = m.ref;
        opt.textContent = m.providerName + ' / ' + m.displayName;
        sel.appendChild(opt);
      });
    }
    function addApiModelRow(container, m) {
      m = m || { id: '', name: '' };
      var row = document.createElement('div');
      row.style.cssText = 'display:flex;gap:4px;align-items:center;';
      var idInput = document.createElement('input');
      idInput.className = 'glass-input';
      idInput.placeholder = 'Model ID (e.g. gpt-4o)';
      idInput.value = m.id || '';
      idInput.style.cssText = 'flex:1;min-width:0';
      ...
      rmBtn.className = 'cmd-item-del';
      rmBtn.textContent = '×';
      rmBtn.title = 'Remove model';
    ...
    var apiAddProvBtn = document.getElementById('apiAddProvBtn');
    var apiAddForm = document.getElementById('apiAddForm');
    var apiNameInput, apiIdInput, apiBaseInput, apiKeyInput;
    if (apiAddProvBtn) apiAddProvBtn.addEventListener('click', function () {
      if (!apiAddForm) return;
      apiAddForm.style.display = 'block';
      apiNameInput = document.getElementById('apiName');
      apiIdInput = document.getElementById('apiId');
      ...
      apiBaseInput.value = '';
      apiKeyInput.value = '';
      apiNameInput.oninput = function () {
        if (!apiIdInput._touched) apiIdInput.value = slugify(apiNameInput.value);
      };
      apiIdInput.oninput = function () { apiIdInput._touched = true; };
      var modelsContainer = document.getElementById('apiModels');
      modelsContainer.innerHTML = '';
      addApiModelRow(modelsContainer);
    });
    var apiAddModelBtn = document.getElementById('apiAddModelBtn');
    if (apiAddModelBtn) apiAddModelBtn.addEventListener('click', function () {
      addApiModelRow(document.getElementById('apiModels'));
    });
    var apiSaveBtn = document.getElementById('apiSaveBtn');
    if (apiSaveBtn) apiSaveBtn.addEventListener('click', function () {
      var name = apiNameInput.value.trim();
      var id = apiIdInput.value.trim() || slugify(name);
      var base = apiBaseInput.value.trim();
      var key = apiKeyInput.value.trim();
      if (!name || !id || !base) { alert('Provider name, ID and Base URL are required.'); return; }
      var modelRows = document.getElementById('apiModels').children;
      var models = [];
      for (var i = 0; i < modelRows.length; i++) {
        var inputs = modelRows[i].querySelectorAll('input.glass-input');
        var mid = inputs[0].value.trim();
        var mname = inputs[1].value.trim();
        if (mid) models.push({ id: mid, name: mname || mid });
      }
      if (models.length === 0) { alert('At least one model is required.'); return; }
      vscode.postMessage({ type: 'addApiProvider', input: { providerId: id, displayName: name, baseURL: base, apiKey: key, models: models } });
      apiAddForm.style.display = 'none';
    });
    var apiCancelBtn = document.getElementById('apiCancelBtn');
    if (apiCancelBtn) apiCancelBtn.addEventListener('click', function () {
      apiAddForm.style.display = 'none';
    });
    var apiActiveModel = document.getElementById('apiActiveModel');
    if (apiActiveModel) apiActiveModel.addEventListener('change', function () {
      vscode.postMessage({ type: 'setActiveModel', modelRef: this.value });
    });
    // Initial load
    vscode.postMessage({ type: 'getShellProxyStatus' });
    vscode.postMessage({ type: 'listApiProviders' });
    vscode.postMessage({ type: 'loadApiModels' });
    vscode.postMessage({ type: 'getConfig' });
  </script>
</body>
</html>`}};function Pe(){let o="",e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let n=0;n<32;n++)o+=e.charAt(Math.floor(Math.random()*e.length));return o}var ue=T(require("http"));var K=class{constructor(e){this._server=null;this._port=e;this._selectedCell=-1;this._busy=!1;this._idleTimer=null;this._idleTimeout=10e3}_markActive(){this._busy=!0;this._idleTimer&&clearTimeout(this._idleTimer);this._idleTimer=setTimeout(()=>{this._busy=!1},this._idleTimeout)}start(e=10){return new Promise((n,a)=>{this._server=this._createServer();let s=t=>{this._server.removeAllListeners("error"),this._server.on("error",r=>{r.code==="EADDRINUSE"&&t<e?(this._port++,s(t+1)):a(r)}),this._server.listen(this._port,"127.0.0.1",()=>{let r=this._server.address();this._port=r.port,n(this._port)})};s(0)})}_createServer(){return ue.createServer((e,n)=>{if(n.setHeader("Content-Type","application/json"),e.method==="OPTIONS"){n.writeHead(204),n.end();return}let a=new URL(e.url||"/",`http://127.0.0.1:${this._port}`);e.method==="GET"&&a.pathname==="/api/health"?(n.writeHead(200),n.end(JSON.stringify({status:"ok"}))):e.method==="GET"&&a.pathname==="/api/info"?this._handleInfo(n):e.method==="GET"&&a.pathname==="/api/diag"?this._handleDiag(n):e.method==="POST"&&a.pathname==="/api/send"?(this._markActive(),this._readBody(e).then(s=>this._handleSend(s,n))):e.method==="POST"&&a.pathname==="/api/read"?(this._markActive(),this._readBody(e).then(s=>this._handleRead(s,n))):e.method==="POST"&&a.pathname==="/api/broadcast"?(this._markActive(),this._readBody(e).then(s=>this._handleBroadcast(s,n))):e.method==="POST"&&a.pathname==="/api/exec"?(this._markActive(),this._readBody(e).then(s=>this._handleExec(s,n))):(n.writeHead(404),n.end(JSON.stringify({error:"Not found"})))})}stop(){this._server?.close(),this._server=null}getPort(){return this._port}_handleInfo(e){let n=w.getActive(),a=w.entries().map(([s,t])=>({tabId:s,rows:t.getRows(),cols:t.getCols(),cellIds:t.getCellIds(),labels:t.getCellLabels()}));e.writeHead(200),e.end(JSON.stringify({grid:n?{rows:n.getRows(),cols:n.getCols(),cellCount:n.getCellCount(),cellLabels:n.getCellLabels()}:null,tabs:a,activeTabId:w.getActiveTabId()??null}))}_handleDiag(t){let r={server:{port:this._port,selectedCell:this._selectedCell},tabs:[]};try{r.tabs=w.entries().map(([n,a])=>({tabId:n,rows:a.getRows(),cols:a.getCols(),cellCount:a.getCellCount(),cells:Array.from({length:a.getRows()*a.getCols()},(s,o)=>({localId:o,hasTerminal:!!a._terminals[o],bufLen:(a._outputBuffers[o]||"").length}))}))}catch(n){r.error=n.message}t.writeHead(200),t.end(JSON.stringify(r))}_handleSend(e,n){let a=typeof e.cellId=="number"?e.cellId:-1;if(a<0)a=this._selectedCell;if(a<0){n.writeHead(200),n.end(JSON.stringify({opencode:!0,message:"Execute in opencode directly"}));return}let s=typeof e.text=="string"?e.text:"",t=e.submit===!0,r=L.resolve(a);if(!r){n.writeHead(200),n.end(JSON.stringify({success:!1,error:"Invalid cell id"}));return}let g=w.get(r.tabId);if(!g){n.writeHead(200),n.end(JSON.stringify({success:!1,error:"Tab no longer open"}));return}let l=g.sendToCell(r.localCellId,t?s+"\r":s);n.writeHead(200),n.end(JSON.stringify({success:l}))}_handleRead(e,n){let a=typeof e.cellId=="number"?e.cellId:-1;if(a<0)a=this._selectedCell;if(a<0){n.writeHead(200),n.end(JSON.stringify({opencode:!0,message:"Execute in opencode directly"}));return}let s=typeof e.lines=="number"?e.lines:void 0,t=L.resolve(a);if(!t){n.writeHead(200),n.end(JSON.stringify({output:null,error:"Invalid cell id"}));return}let r=w.get(t.tabId);if(!r){n.writeHead(200),n.end(JSON.stringify({output:null,error:"Tab no longer open"}));return}let g=r.readCell(t.localCellId,s);n.writeHead(200),n.end(JSON.stringify({output:g}))}_handleBroadcast(e,n){let a=w.getActive();if(!a){n.writeHead(200),n.end(JSON.stringify({success:!1,error:"No grid open"}));return}let s=typeof e.text=="string"?e.text:"",t=e.submit===!0,r=a.getCellCount();if(t)a.broadcastInput(s);else for(let g=0;g<r;g++)a.sendToCell(g,s);n.writeHead(200),n.end(JSON.stringify({success:!0,cellCount:r}))}async _handleExec(e,n){
try{
let a=typeof e.cellId=="number"?e.cellId:-1;
if(a<0)a=this._selectedCell;
if(a<0){n.writeHead(200);n.end(JSON.stringify({opencode:true,message:"Execute in opencode directly"}));return}
const s=typeof e.command=="string"?e.command:"";
const t=typeof e.timeout=="number"?e.timeout:0;
const r=L.resolve(a);
if(!r){n.writeHead(200);n.end(JSON.stringify({output:null,error:"Invalid cell id"}));return}
const g=w.get(r.tabId);
if(!g){n.writeHead(200);n.end(JSON.stringify({output:null,error:"Tab no longer open"}));return}
const shellType=(g._cellShellType&&g._cellShellType[r.localCellId])||"cmd";
const probeId=Math.random().toString(36).slice(2,10);
const marker="TG_EXIT_"+probeId;
let probe="";
if(shellType==="pwsh"||shellType==="powershell"){
probe=`; Write-Output "${marker}:$LASTEXITCODE"`;
}else if(shellType==="bash"||shellType==="sh"){
probe=`; printf '\n${marker}:%d\n' "$?"`;
}else{
probe=`& echo ${marker}:%ERRORLEVEL%`;
}
...
}_readBody(e){return new Promise(n=>{let a="";e.on("data",s=>{a+=s}),e.on("end",()=>{try{n(JSON.parse(a))}catch{n({})}})})}};var H,j,_ocgDataDir="";async function Oe(o){if(p.init(o),await p.migrateOnce(),o.globalState.get("lastTabs",[]).length===0){let d=o.globalState.get("lastGrid");if(d&&d.rows>0&&d.cols>0){let f=d.rows*d.cols,v=[];for(let S=0;S<f;S++)v.push(S);await o.globalState.update("lastTabs",[{tabId:0,rows:d.rows,cols:d.cols,cellIds:v}]),o.globalState.get("nextTabId",0)<1&&await o.globalState.update("nextTabId",1),o.globalState.get("nextGlobalCellId",0)<f&&await o.globalState.update("nextGlobalCellId",f)}}try{_ocgDataDir=U.join(o.extensionPath,"data");let d=_ocgDataDir,f=U.join(d,"reload-signal");P.mkdirSync(d,{recursive:!0});let v=P.existsSync(f)?P.statSync(f).mtimeMs:0,c=P.watch(d,(h,S)=>{if(S==="reload-signal")try{let C=P.statSync(f).mtimeMs;C>v&&(v=C,u.commands.executeCommand("workbench.action.reloadWindow"))}catch{}});o.subscriptions.push({dispose:()=>c.close()})}catch{}try{let oldDir=U.join(me.homedir(),".opencode-grid");if(P.existsSync(oldDir))P.rmSync(oldDir,{recursive:!0,force:!0})}catch{}let n=u.workspace.workspaceFolders?.[0]?.uri.fsPath;if(n){let f=o.globalState.get("projectPresets",{})[n];if(f){let c=o.globalState.get("presets",[]).find(h=>h.name===f);if(c){let h=u.workspace.getConfiguration("opencodeGrid");if(await h.update("defaultRows",c.rows,u.ConfigurationTarget.Global),await h.update("defaultCols",c.cols,u.ConfigurationTarget.Global),await h.update("zoomPercent",c.zoomPercent,u.ConfigurationTarget.Global),await h.update("fontFamily",c.fontFamily,u.ConfigurationTarget.Global),await h.update("backgroundColor",c.bgColor,u.ConfigurationTarget.Global),await h.update("foregroundColor",c.fgColor,u.ConfigurationTarget.Global),await h.update("colorTheme",c.colorTheme||"",u.ConfigurationTarget.Global),await h.update("shellType",c.shellType||"",u.ConfigurationTarget.Global),o.globalState.get("lastTabs",[]).length===0){let C=E.next(o);if(await p.setStartupCommands(C,c.startupCommands||[]),await p.setCellLabels(C,c.cellLabels||[]),await p.setDefaultCommand(C,c.defaultCommand||""),c.defaultSteps?await p.setDefaultSteps(C,c.defaultSteps):c.defaultCommand?await p.setDefaultSteps(C,[{type:"command",input:c.defaultCommand}]):await p.setDefaultSteps(C,[]),c.cellStepsOverrides){let I={};for(let[k,D]of Object.entries(c.cellStepsOverrides))I[Number(k)]={},Array.isArray(D.startupSteps)&&(I[Number(k)].startupSteps=D.startupSteps);await p.setCellOverrides(C,I)}await p.setMergedRegions(C,c.mergedRegions||[]),await o.globalState.update("pendingFirstTabId",C)}}}}let a=new z(o),s=u.workspace.getConfiguration("opencodeGrid").get("apiPort",7890);s>0&&(H=new K(s),H.start().then(d=>{j=u.window.createStatusBarItem(u.StatusBarAlignment.Right,50),j.text=`$(broadcast) OCG :${d}`,j.tooltip=u.l10n.t("OpenCode Grid API active on port {0}",d),j.show(),o.subscriptions.push(j);try{let wp=u.workspace.workspaceFolders?.[0]?.uri.fsPath||process.cwd();let crypto=require("crypto");let workspaceHash=crypto.createHash("md5").update(wp).digest("hex").slice(0,8);let vscodeWindowPid=process.ppid||0;let dir=U.join(_ocgDataDir,"ports");P.mkdirSync(dir,{recursive:!0});let portFile=U.join(dir,`win-${vscodeWindowPid}-${workspaceHash}.json`);let portData={port:d,vscodeWindowPid,extHostPid:process.pid,workspace:wp,ts:Date.now()};P.writeFileSync(portFile,JSON.stringify(portData),"utf-8");let heartbeat=setInterval(()=>{try{portData.ts=Date.now();P.writeFileSync(portFile,JSON.stringify(portData),"utf-8")}catch(e){}},30000);let _disposed=false;o.subscriptions.push({dispose(){if(_disposed)return;_disposed=true;try{clearInterval(heartbeat)}catch(e){}try{P.unlinkSync(portFile)}catch(e){}}})}catch(e){}}).catch(d=>{u.window.showWarningMessage(u.l10n.t("OpenCode Grid API bridge failed to start: {0}",d.message))}));o.subscriptions.push(u.window.registerWebviewViewProvider(z.viewType,a)),o.subscriptions.push(u.commands.registerCommand("opencodeGrid._refreshSidebar",()=>{a.sendConfig()}));let r=o.globalState.get("lastTabs",[]),g=0,l=!1,b=!1,m=async()=>{if(l)return;l=!0,w.size()>0&&x.persistTabs(o);let d=new Set(w.entries().map(([v])=>v)),f=r.filter(v=>!d.has(v.tabId));if(f.length>0){let v=[];for(let c of u.window.tabGroups.all)for(let h of c.tabs)if(h.input instanceof u.TabInputWebview){let S=h.input.viewType||"";(S==="opencodeGrid"||S.endsWith("-opencodeGrid"))&&!h.isActive&&v.push(h)}if(v.length>0)try{await u.window.tabGroups.close(v)}catch{}for(let c of f)x.createOrShow(o,c.rows,c.cols,{forceNewTab:!0,tabIdOverride:c.tabId,cellIdsOverride:c.cellIds})}else w.size()===0&&o.globalState.get("lastTabs",[]).length>0&&(o.globalState.update("lastTabs",void 0),o.globalState.update("lastGrid",void 0))};o.subscriptions.push(u.window.registerWebviewPanelSerializer("opencodeGrid",{async deserializeWebviewPanel(d,f){if(g<r.length){let v=r[g++];x.revive(d,o,v.rows,v.cols,v.tabId,v.cellIds)}else d.dispose();b||(b=!0,setTimeout(()=>{m()},100))}})),setTimeout(()=>{m()},1500),o.subscriptions.push(u.commands.registerCommand("opencodeGrid.openGrid",()=>{let d=u.workspace.getConfiguration("opencodeGrid"),f=d.get("defaultRows",2),v=d.get("defaultCols",3);x.createOrShow(o,f,v)}),u.commands.registerCommand("opencodeGrid.openCustomGrid",(d,f)=>{x.createOrShow(o,d,f)}),u.commands.registerCommand("opencodeGrid.open2x2",()=>x.createOrShow(o,2,2)),u.commands.registerCommand("opencodeGrid.open2x3",()=>x.createOrShow(o,2,3)),u.commands.registerCommand("opencodeGrid.open3x3",()=>x.createOrShow(o,3,3)),u.commands.registerCommand("opencodeGrid.newTab",()=>{let d=w.getActive(),f=u.workspace.getConfiguration("opencodeGrid"),v=d?.getRows()??f.get("defaultRows",2),c=d?.getCols()??f.get("defaultCols",3);x.createOrShow(o,v,c,{forceNewTab:!0})}),u.commands.registerCommand("opencodeGrid.duplicateTab",async()=>{let d=w.getActive();if(!d){u.window.showWarningMessage(u.l10n.t("No active tab to duplicate."));return}let f=d.getRows(),v=d.getCols(),c=d.getTabId(),h=E.next(o);await p.cloneTab(c,h),x.createOrShow(o,f,v,{forceNewTab:!0,tabIdOverride:h}),u.window.showInformationMessage(u.l10n.t("Tab duplicated. Terminal history is not copied; cells will start with the configured startup commands."))}),u.commands.registerCommand("opencodeGrid.closeTab",()=>{if(w.size()<=1){u.window.showWarningMessage(u.l10n.t("Cannot close the last remaining tab."));return}let d=w.getActive();d&&d.dispose()}),u.commands.registerCommand("opencodeGrid.resetCellIds",async()=>{if(w.size()>0){u.window.showWarningMessage(u.l10n.t("Close all OpenCode Grid tabs before resetting cell IDs."));return}await L.reset(o),await E.reset(o),u.window.showInformationMessage(u.l10n.t("Cell IDs and tab counter reset."))}),u.commands.registerCommand("opencodeGrid.resetAllTabs",async()=>{await u.window.showWarningMessage(u.l10n.t("Close all OpenCode Grid tabs and wipe persisted tab state? This cannot be undone."),{modal:!0},u.l10n.t("Reset"))===u.l10n.t("Reset")&&(w.disposeAll(),await L.reset(o),await E.reset(o),await o.globalState.update("lastTabs",void 0),await o.globalState.update("lastGrid",void 0),await o.globalState.update("pendingFirstTabId",void 0),u.window.showInformationMessage(u.l10n.t("All OpenCode Grid tabs and persisted state reset.")))}),u.commands.registerCommand("opencodeGrid.sendToCell",(d,f)=>{let v=L.resolve(d);return v?w.get(v.tabId)?.sendToCell(v.localCellId,f)??!1:!1}),u.commands.registerCommand("opencodeGrid.readCell",(d,f)=>{let v=L.resolve(d);return v?w.get(v.tabId)?.readCell(v.localCellId,f)??null:null}),u.commands.registerCommand("opencodeGrid.getGridInfo",()=>{let d=w.getActive();if(!d)return null;let f=w.entries().map(([v,c])=>({tabId:v,rows:c.getRows(),cols:c.getCols(),cellIds:c.getCellIds(),labels:c.getCellLabels()}));return{rows:d.getRows(),cols:d.getCols(),cellCount:d.getCellCount(),cellLabels:d.getCellLabels(),tabs:f,activeTabId:w.getActiveTabId()??null}}),u.commands.registerCommand("opencodeGrid.setupOpencode",async()=>{let cfgDir=R.join(Y.homedir(),".config","opencode");let cfgPath=R.join(cfgDir,"opencode.json");let shellPath=U.join(o.extensionPath,"tg-shell.cmd");try{_.existsSync(cfgDir)||_.mkdirSync(cfgDir,{recursive:!0});let j={};if(_.existsSync(cfgPath)){try{j=JSON.parse(_.readFileSync(cfgPath,"utf-8"))}catch{j={}}}j.shell=shellPath;_.writeFileSync(cfgPath,JSON.stringify(j,null,2),"utf-8");u.window.showInformationMessage(u.l10n.t("OpenCode configured to use this OpenCode Grid shell. Restart opencode to activate."))}catch(e){u.window.showErrorMessage(u.l10n.t("Failed to setup OpenCode: {0}",String(e instanceof Error?e.message:e)))}}),u.commands.registerCommand("opencodeGrid.disableShellProxy",async()=>{let cfgPath=R.join(Y.homedir(),".config","opencode","opencode.json");try{if(!_.existsSync(cfgPath)){u.window.showInformationMessage(u.l10n.t("opencode.json not found. OpenCode Grid proxy is not active."));return}let j={};try{j=JSON.parse(_.readFileSync(cfgPath,"utf-8"))}catch(e){u.window.showErrorMessage(u.l10n.t("Failed to parse opencode.json: {0}",String(e)));return}let curShell=typeof j.shell==="string"?j.shell:"";let isGridProxy=curShell.toLowerCase().endsWith("tg-shell.cmd");if(!isGridProxy){u.window.showInformationMessage(u.l10n.t("OpenCode Grid proxy is not active (shell is not tg-shell.cmd)."));return}try{_.copyFileSync(cfgPath,cfgPath+".bak")}catch{}delete j.shell;_.writeFileSync(cfgPath,JSON.stringify(j,null,2),"utf-8");u.window.showInformationMessage(u.l10n.t("OpenCode Grid proxy disabled. Restart opencode to use default PowerShell."))}catch(e){u.window.showErrorMessage(u.l10n.t("Failed to disable proxy: {0}",String(e instanceof Error?e.message:e)))}}),u.commands.registerCommand("opencodeGrid.listCustomProviders",async()=>{try{let cfgPath=R.join(Y.homedir(),".config","opencode","opencode.json");if(!_.existsSync(cfgPath))return[];let j={};try{j=JSON.parse(_.readFileSync(cfgPath,"utf-8"))}catch{return[]}let providers=j.provider||{};let result=[];for(let pid of Object.keys(providers)){let p=providers[pid];if(!p||typeof p!=="object")continue;let models=[];if(p.models&&typeof p.models==="object"){for(let mid of Object.keys(p.models)){let m=p.models[mid]||{};models.push({id:mid,name:m.name||mid,contextLimit:m.limit?.context,outputLimit:m.limit?.output})}}result.push({providerId:pid,displayName:p.name||pid,npm:p.npm||"",baseURL:p.options?.baseURL||"",apiKey:p.options?.apiKey||"",customHeaders:p.options?.headers||{},models})}return result}catch(e){u.window.showErrorMessage(u.l10n.t("Failed to list providers: {0}",String(e instanceof Error?e.message:e)));return[]}}),u.commands.registerCommand("opencodeGrid.addCustomProvider",async(input)=>{if(!input||typeof input!=="object"||!input.providerId||!input.baseURL||!Array.isArray(input.models)||input.models.length===0){u.window.showErrorMessage(u.l10n.t("Invalid provider input. providerId, baseURL and at least one model are required."));return}try{let cfgDir=R.join(Y.homedir(),".config","opencode");let cfgPath=R.join(cfgDir,"opencode.json");_.existsSync(cfgDir)||_.mkdirSync(cfgDir,{recursive:!0});let j={};if(_.existsSync(cfgPath)){try{j=JSON.parse(_.readFileSync(cfgPath,"utf-8"))}catch{j={}};try{_.copyFileSync(cfgPath,cfgPath+".bak")}catch{}}if(!j.provider)j.provider={};let providerData={npm:input.npm||"@ai-sdk/openai-compatible",name:input.displayName||input.providerId,options:{baseURL:input.baseURL},models:{}};if(input.apiKey){providerData.options.apiKey=input.apiKey}if(input.customHeaders&&typeof input.customHeaders==="object"&&Object.keys(input.customHeaders).length>0){providerData.options.headers=input.customHeaders}for(let m of input.models){let modelData={name:m.name||m.id};if(m.contextLimit||m.outputLimit){modelData.limit={context:m.contextLimit||128000,output:m.outputLimit||8192}}providerData.models[m.id]=modelData}j.provider[input.providerId]=providerData;_.writeFileSync(cfgPath,JSON.stringify(j,null,2),"utf-8");u.window.showInformationMessage(u.l10n.t("Provider {0} added. Restart opencode to activate.",input.providerId))}catch(e){u.window.showErrorMessage(u.l10n.t("Failed to add provider: {0}",String(e instanceof Error?e.message:e)))}}),u.commands.registerCommand("opencodeGrid.editCustomProvider",async(providerId,input)=>{if(!providerId||!input){u.window.showErrorMessage(u.l10n.t("providerId and input are required."));return}try{let cfgPath=R.join(Y.homedir(),".config","opencode","opencode.json");if(!_.existsSync(cfgPath)){u.window.showErrorMessage(u.l10n.t("opencode.json not found."));return}let j={};try{j=JSON.parse(_.readFileSync(cfgPath,"utf-8"))}catch(e){u.window.showErrorMessage(u.l10n.t("Failed to parse opencode.json: {0}",String(e)));return}if(!j.provider||!j.provider[providerId]){u.window.showErrorMessage(u.l10n.t("Provider {0} not found.",providerId));return}try{_.copyFileSync(cfgPath,cfgPath+".bak")}catch{}let p=j.provider[providerId];if(input.displayName)p.name=input.displayName;if(input.baseURL)p.options.baseURL=input.baseURL;if(input.apiKey!==undefined)p.options.apiKey=input.apiKey;if(input.customHeaders&&typeof input.customHeaders==="object"){if(Object.keys(input.customHeaders).length>0)p.options.headers=input.customHeaders;else delete p.options.headers}if(input.npm)p.npm=input.npm;if(Array.isArray(input.models)){let newModels={};for(let m of input.models){let modelData={name:m.name||m.id};if(m.contextLimit||m.outputLimit){modelData.limit={context:m.contextLimit||128000,output:m.outputLimit||8192}}newModels[m.id]=modelData}p.models=newModels}_.writeFileSync(cfgPath,JSON.stringify(j,null,2),"utf-8");u.window.showInformationMessage(u.l10n.t("Provider {0} updated. Restart opencode to activate.",providerId))}catch(e){u.window.showErrorMessage(u.l10n.t("Failed to edit provider: {0}",String(e instanceof Error?e.message:e)))}}),u.commands.registerCommand("opencodeGrid.removeCustomProvider",async(providerId)=>{if(!providerId){u.window.showErrorMessage(u.l10n.t("providerId is required."));return}try{let cfgPath=R.join(Y.homedir(),".config","opencode","opencode.json");if(!_.existsSync(cfgPath))return;let j={};try{j=JSON.parse(_.readFileSync(cfgPath,"utf-8"))}catch{return}if(!j.provider||!j.provider[providerId]){u.window.showWarningMessage(u.l10n.t("Provider {0} not found.",providerId));return}try{_.copyFileSync(cfgPath,cfgPath+".bak")}catch{}delete j.provider[providerId];if(j.model&&typeof j.model==="string"&&j.model.startsWith(providerId+"/")){delete j.model}if(j.small_model&&typeof j.small_model==="string"&&j.small_model.startsWith(providerId+"/")){delete j.small_model}_.writeFileSync(cfgPath,JSON.stringify(j,null,2),"utf-8");u.window.showInformationMessage(u.l10n.t("Provider {0} removed. Restart opencode to activate.",providerId))}catch(e){u.window.showErrorMessage(u.l10n.t("Failed to remove provider: {0}",String(e instanceof Error?e.message:e)))}}),u.commands.registerCommand("opencodeGrid.setActiveModel",async(modelRef)=>{if(!modelRef||typeof modelRef!=="string"){u.window.showErrorMessage(u.l10n.t("modelRef is required (format: provider-id/model-id)."));return}try{let cfgPath=R.join(Y.homedir(),".config","opencode","opencode.json");let j={};if(_.existsSync(cfgPath)){try{j=JSON.parse(_.readFileSync(cfgPath,"utf-8"))}catch{j={}};try{_.copyFileSync(cfgPath,cfgPath+".bak")}catch{}}else{let cfgDir=R.join(Y.homedir(),".config","opencode");_.mkdirSync(cfgDir,{recursive:!0})}j.model=modelRef;_.writeFileSync(cfgPath,JSON.stringify(j,null,2),"utf-8");u.window.showInformationMessage(u.l10n.t("Active model set to {0}. Restart opencode to activate.",modelRef))}catch(e){u.window.showErrorMessage(u.l10n.t("Failed to set active model: {0}",String(e instanceof Error?e.message:e)))}}),u.commands.registerCommand("opencodeGrid.listAllModels",async()=>{try{let cfgPath=R.join(Y.homedir(),".config","opencode","opencode.json");if(!_.existsSync(cfgPath))return[];let j={};try{j=JSON.parse(_.readFileSync(cfgPath,"utf-8"))}catch{return[]}let providers=j.provider||{};let result=[];let disabledProviders=new Set(j.disabled_providers||[]);for(let pid of Object.keys(providers)){if(disabledProviders.has(pid))continue;let p=providers[pid];if(!p||typeof p!=="object")continue;if(p.models&&typeof p.models==="object"){for(let mid of Object.keys(p.models)){let m=p.models[mid]||{};result.push({providerId:pid,modelId:mid,ref:pid+"/"+mid,displayName:m.name||mid,providerName:p.name||pid})}}}return result}catch(e){u.window.showErrorMessage(u.l10n.t("Failed to list models: {0}",String(e instanceof Error?e.message:e)));return[]}}),u.commands.registerCommand("opencodeGrid.testAPI",async()=>{let d=u.window.createOutputChannel("OpenCode Grid Tests");d.show(),d.appendLine(`=== OpenCode Grid API Tests ===
`);let f=0,v=0;function c(M,B,N){let F=B?"PASS":"FAIL";B?f++:v++,d.appendLine(`[${F}] ${M}${N?" \u2014 "+N:""}`)}let h=await u.commands.executeCommand("opencodeGrid.getGridInfo");if(!h){d.appendLine("[FAIL] getGridInfo returned null. Open a grid first.");return}c("getGridInfo returns object",!!h,JSON.stringify(h)),c("rows is number",typeof h.rows=="number",`rows=${h.rows}`),c("cols is number",typeof h.cols=="number",`cols=${h.cols}`),c("cellCount = rows*cols",h.cellCount===h.rows*h.cols,`${h.cellCount}`),c("cellLabels is array",Array.isArray(h.cellLabels),`length=${h.cellLabels.length}`),c("cellLabels.length = cellCount",h.cellLabels.length===h.cellCount);let S=await u.commands.executeCommand("opencodeGrid.sendToCell",0,"echo __API_TEST__\r");c("sendToCell(0) returns true",S===!0);let C=await u.commands.executeCommand("opencodeGrid.sendToCell",999,"x\r");c("sendToCell(999) returns false",C===!1,`got ${C}`);let I=await u.commands.executeCommand("opencodeGrid.sendToCell",0,"TYPED_ONLY");c("sendToCell without \\r returns true",I===!0),await new Promise(M=>setTimeout(M,2e3)),await u.commands.executeCommand("opencodeGrid.sendToCell",0,"");let k=await u.commands.executeCommand("opencodeGrid.readCell",0);c("readCell(0) returns string",typeof k=="string",`length=${k?.length??0}`),c("readCell(0) contains test marker",!!k&&k.includes("__API_TEST__"));let D=await u.commands.executeCommand("opencodeGrid.readCell",0,3);c("readCell(0, 3) returns string",typeof D=="string");let ee=await u.commands.executeCommand("opencodeGrid.readCell",0,0);c("readCell(0, 0) returns empty",ee==="",`got "${ee}"`);let te=await u.commands.executeCommand("opencodeGrid.readCell",999);if(c("readCell(999) returns null",te===null,`got ${te}`),h.cellCount>1){let M=await u.commands.executeCommand("opencodeGrid.sendToCell",1,"echo CELL1_OK\r");c("sendToCell(1) returns true",M===!0),await new Promise(N=>setTimeout(N,1500));let B=await u.commands.executeCommand("opencodeGrid.readCell",1);c("readCell(1) contains CELL1_OK",!!B&&B.includes("CELL1_OK"))}if(h.tabs&&h.tabs.length>1){d.appendLine(`
--- Multi-tab tests ---`);let M=h.tabs.flatMap(V=>V.cellIds),B=new Set(M);c("global cell ids unique across all tabs",B.size===M.length,`${M.length} ids`),c("activeTabId is a number",typeof h.activeTabId=="number");let N=h.tabs[1],F=N.cellIds[0],be=await u.commands.executeCommand("opencodeGrid.sendToCell",F,"echo __MULTITAB_OK__\r");c(`sendToCell global=${F} (tab ${N.tabId+1} cell 1) returns true`,be===!0),await new Promise(V=>setTimeout(V,1500));let se=await u.commands.executeCommand("opencodeGrid.readCell",F);c(`readCell global=${F} contains __MULTITAB_OK__`,!!se&&se.includes("__MULTITAB_OK__"));let ne=Math.max(...M)+1e4,fe=await u.commands.executeCommand("opencodeGrid.sendToCell",ne,"x");c(`sendToCell with bogus global id=${ne} returns false`,fe===!1)}else h.tabs&&d.appendLine(`
(Multi-tab tests skipped: only ${h.tabs.length} tab open. Open a second tab via the sidebar to enable.)`);d.appendLine(`
=== ${f} passed, ${v} failed ===`),v===0?u.window.showInformationMessage(u.l10n.t("OpenCode Grid API: All {0} tests passed!",f)):u.window.showWarningMessage(u.l10n.t("OpenCode Grid API: {0} test(s) failed. See output.",v))}),)}function Be(){try{if(!_ocgDataDir)_ocgDataDir=U.join(__dirname,"..","data");let dir=U.join(_ocgDataDir,"ports");if(P.existsSync(dir)){let files=P.readdirSync(dir).filter(f=>f.startsWith("win-")&&f.endsWith(".json"));for(let f of files)try{let fp=U.join(dir,f);let d=JSON.parse(P.readFileSync(fp,"utf-8"));if(d.extHostPid===process.pid)P.unlinkSync(fp)}catch(e){}}}catch(e){}H?.stop(),H=void 0,w.disposeAll()}0&&(module.exports={activate,deactivate});
